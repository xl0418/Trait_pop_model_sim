...
def DVParamLiang(gamma, a, K, h, nu, r, theta, V00, V01, Vmax, inittrait, initpop, initpop_sigma, break_on_mu, num_threads=-1):
    return np.array([gamma, a, K, h, nu, r, theta, V00, V01, Vmax, inittrait, initpop, initpop_sigma, 1.0 if break_on_mu else 0.0, num_threads]).astype(float)

# converts named parameters in numpy.array
# deprecated 
def DVParamDrury(gamma, a, theta, m, inittrait, var_trait, num_threads=-1):
    return np.array([gamma, a, theta, m, inittrait, var_trait, num_threads]).astype(float)
