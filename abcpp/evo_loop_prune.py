...
for r in range(10000):
    obs = dvcpp.DVSim(td_full, obs_param)
    if obs['sim_time'] == td_full.sim_evo_time:
        break
if obs['sim_time'] < td_full.sim_evo_time:
...
obsZ = obs['Z'][s]
obsV = obs['V'][s]
index_extant = np.where(~np.isnan(obsZ))[0]
obsZ = obsZ[~np.isnan(obsZ)]
obsN = obsN[index_extant]
obsV = obsV[~np.isnan(obsV)]
...
for g in range(generations):
    ...
    a_data[g, :] = params[:, 1]
    nu_data[g,:] = params[:,3]
    pop = dvcpp.DVSim(td_purne, params)

    # access fitness
    # fitness = np.zeros(population)
    valid = np.where(pop['sim_time'] == td_purne.sim_evo_time)[0]
    if len(valid)<20:
